...
def download_youtube_video(url):
    try:
        ...
        original_title = selected_stream.title
        processed_title = re.sub(r'[\'\"\s]', '', original_title)  # Elimina comillas simples, dobles y espacios
        #filename = f"video_{processed_title}"

        if not os.path.exists('videos'):
            os.makedirs('videos')

        print(f"Downloading video: {yt.title}")
        video_file = selected_stream.download(output_path='videos', filename=processed_title, filename_prefix="video_")

        if not selected_stream.is_progressive:
            print("Downloading audio...")
            audio_file = audio_stream.download(output_path='videos', filename=processed_title, filename_prefix="audio_", )

            print("Merging video and audio...")
            output_file = os.path.join('videos', f"{processed_title}.mp4")
            stream = ffmpeg.input(video_file)
            audio = ffmpeg.input(audio_file)
            stream = ffmpeg.output(stream, audio, output_file, vcodec='libx264', acodec='aac', strict='experimental')
            ffmpeg.run(stream, overwrite_output=True, cmd='C:\\ffmpeg\\bin\\ffmpeg.exe')
